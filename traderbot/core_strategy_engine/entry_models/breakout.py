...
def evaluate_breakout(features, config=None):
    ...
    no_chase = bar["c"] <= recent_high + float(
        settings.get("maximum_chase_atr", 1.0)
    ) * atr
    checks = {
    ...
    factor_scores = {
        "price_action": average_check_score(breakout_now),
        "stock_trend": average_check_score(trend_ok),
        ...
        "relative_strength_execution": average_check_score(no_chase),
    }
